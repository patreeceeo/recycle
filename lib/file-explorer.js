...
(function () {
  ...
  var DebounceAttributes = function (options) {
    if (!(this instanceof DebounceAttributes))
      return new DebounceAttributes(options);
    ...
    var MainIntervalHandler = function () {
      ...
      if (nummatches < $this.settings.watchers.length) {
        ...
        if ($this.settings.intervalcallback)
          $this.settings.intervalcallback.call($this);
      } else {
        numsame++;
  ...
  function GetFileExplorerDragIconData(e, group) {
    ...
    for (var x = 0; x < e.dataTransfer.types.length; x++) {
      if (
        e.dataTransfer.types[x].startsWith('application/file-explorer-icon;')
      ) {
        try {
  ...
  function OverlayDragOverHandler(e) {
    if (
      ...
      fe_dragiconoverlay.style.left =
        e.clientX - fe_dragiconoverlaypos.xdiff + 'px';
      fe_dragiconoverlay.style.top =
        e.clientY - fe_dragiconoverlaypos.ydiff + 'px';
    }
  }
  ...
  var PrepareXHR = function (options) {
    ...
    if (options.onerror) {
      ...
      if (!options.onabort)
        $this.xhr.addEventListener('abort', options.onerror);
      if (!options.ontimeout)
        $this.xhr.addEventListener('timeout', options.onerror);
    }
    ...
    if (options.onprogress)
      $this.xhr.addEventListener('progress', options.onprogress);
    if (options.ontimeout)
      $this.xhr.addEventListener('timeout', options.ontimeout);
    if (options.onloadend)
      $this.xhr.addEventListener('loadend', options.onloadend);
    ...
    $this.upload.addEventListener = function (type, listener, options) {
      if (!sent && $this && $this.xhr)
        $this.xhr.upload.addEventListener(type, listener, options);
    };

    $this.upload.removeEventListener = function (type, listener, options) {
      if ($this && $this.xhr)
        $this.xhr.upload.removeEventListener(type, listener, options);
    };
    ...
    $this.GetMethod = function () {
      return (
        options.method || (options.params || options.body ? 'POST' : 'GET')
      );
    };
    ...
    $this.PrepareBody = function () {
      ...
      if (options.params) {
        if (options.params instanceof FormData) {
          ...
          for (var x = 0; x < options.params.length; x++)
            xhrbody.append(options.params[x].name, options.params[x].value);
        } else {
          for (var x in options.params) {
            if (options.params.hasOwnProperty(x)) {
              if (
                typeof options.params[x] === 'string' ||
                typeof options.params[x] === 'number'
              )
                xhrbody.append(x, options.params[x]);
            }
          }
        }
      }
    ...
    $this.Send = function (xhrbody) {
      ...
      if (options.headers) {
        for (var x in options.headers) {
          if (
            options.headers.hasOwnProperty(x) &&
            typeof options.headers[x] === 'string'
          )
            $this.xhr.setRequestHeader(x, options.headers[x]);
        }
      }
  ...
  var ImageLoader = function (options) {
    ...
    $this.ProcessQueue = function () {
      while (numactive < $this.settings.maxactive && numqueued) {
        while (minqueueid < nextid && !queue.hasOwnProperty(minqueueid))
          minqueueid++;
        ...
        var imgnode =
          opts.width && opts.height
            ? new Image(opts.width, opts.height)
            : new Image();
  ...
  var Folder = function (path) {
    ...
    $this.SetEntries = function (newentries) {
      if ($this.busy) {
        $this.busyqueue.push({
          callback: $this.SetEntries,
          callbackopts: [newentries],
        });
    ...
    $this.UpdateEntries = function (updatedentries) {
      if ($this.busy) {
        $this.busyqueue.push({
          callback: $this.UpdateEntries,
          callbackopts: [updatedentries],
        });
  ...
  var PopupMenu = function (parentelem, options) {
    ...
    var elems = {
      popupwrap: CreateNode('div', ['fe_fileexplorer_popup_wrap'], {
        tabIndex: 0,
      }),
    ...
    for (var x = 0; x < $this.settings.items.length; x++) {
      ...
      if (item === 'split') {
        ...
        var itemnode = CreateNode('div', ['fe_fileexplorer_popup_item_wrap'], {
          tabIndex: -1,
        });
        var itemicon = CreateNode('div', ['fe_fileexplorer_popup_item_icon']);
        var itemiconinner = CreateNode('div', [
        ...
        if (!enabled)
          itemnode.classList.add('fe_fileexplorer_popup_item_disabled');
    ...
    if ($this.settings.onposition)
      $this.addEventListener('position', $this.settings.onposition);
    if ($this.settings.onselchanged)
      $this.addEventListener('selection_changed', $this.settings.onselchanged);
    if ($this.settings.onselected)
      $this.addEventListener('selected', $this.settings.onselected);
    if ($this.settings.oncancel)
      $this.addEventListener('cancelled', $this.settings.oncancel);
    if ($this.settings.onleft)
      $this.addEventListener('left', $this.settings.onleft);
    if ($this.settings.onright)
      $this.addEventListener('right', $this.settings.onright);
    if ($this.settings.ondestroy)
      $this.addEventListener('destroy', $this.settings.ondestroy);
    ...
    var InnerWrapMoveHandler = function (e) {
      ...
      var node = e.target;
      while (node && node.parentNode !== elems.innerwrap)
      ...
      if (node && (lastitem !== node || lastitem !== document.activeElement)) {
        if (node.classList.contains('fe_fileexplorer_popup_item_wrap')) {
          ...
          if (lastitem !== node)
            DispatchEvent('selection_changed', [
              node.dataset.itemid,
              itemidmap[node.dataset.itemid],
            ]);
    ...
    var NotifySelected = function (etype) {
      ...
      DispatchEvent('selected', [
        itemidmap[lastitem.dataset.itemid].id,
        itemidmap[lastitem.dataset.itemid],
        lastactiveelem,
        etype,
      ]);
    };
    ...
    var MainClickHandler = function (e) {
      ...
      if (
        e.button == 0 &&
        lastitem !== false &&
        lastitem === document.activeElement &&
        !lastitem.classList.contains('fe_fileexplorer_popup_item_disabled')
      )
    ...
    var MainKeyHandler = function (e) {
      ...
      if (e.keyCode == 37) {
        ...
        var node =
          lastitem === false
            ? elems.innerwrap.lastChild
            : lastitem.previousSibling;

        while (
          node &&
          !node.classList.contains('fe_fileexplorer_popup_item_wrap')
        )
    ...
    if (
      Array.isArray($this.settings.resizewatchers) &&
      $this.settings.resizewatchers.length
    ) {
  ...
  var TextareaOverlay = function (parentelem, options) {
    if (!(this instanceof TextareaOverlay))
      return new TextareaOverlay(parentelem, options);
    ...
    if ($this.settings.onposition)
      $this.addEventListener('position', $this.settings.onposition);
    if ($this.settings.ondone)
      $this.addEventListener('done', $this.settings.ondone);
    if ($this.settings.oncancel)
      $this.addEventListener('cancelled', $this.settings.oncancel);
    if ($this.settings.ondestroy)
      $this.addEventListener('destroy', $this.settings.ondestroy);
    ...
    var MainKeyHandler = function (e) {
      ...
      if (e.keyCode == 46) {
        ...
        if (!$this.settings.capturetab || e.shiftKey) $this.Done('key');
          ...
          elems.maintext.value =
            elems.maintext.value.substring(0, pos) +
            '\t' +
            elems.maintext.value.substring(elems.maintext.selectionEnd);
    ...
    $this.Done = function (etype) {
      if ($this.settings.initvalue === elems.maintext.value)
        $this.Cancel(etype);
    ...
    elems.maintext.focus();
    elems.maintext.setSelectionRange(
      $this.settings.initselstart > -1
        ? $this.settings.initselstart
        : elems.maintext.value.length,
      $this.settings.initselend > -1
        ? $this.settings.initselend
        : elems.maintext.value.length
    );
    allowcanceldone = true;
  };
  ...
  window.FileExplorer = function (parentelem, options) {
    if (!(this instanceof FileExplorer))
      return new FileExplorer(parentelem, options);
    ...
    var main_group =
      typeof $this.settings.group === 'string' ? $this.settings.group : main_id;
    ...
    var elems = {
      ...
      navtool_back: CreateNode(
        'button',
        ['fe_fileexplorer_navtool_back', 'fe_fileexplorer_disabled'],
        { title: $this.Translate('Back (Alt + Left Arrow)'), tabIndex: -1 }
      ),
      navtool_forward: CreateNode(
        'button',
        ['fe_fileexplorer_navtool_forward', 'fe_fileexplorer_disabled'],
        { title: $this.Translate('Forward (Alt + Right Arrow)'), tabIndex: -1 }
      ),
      navtool_history: CreateNode(
        'button',
        ['fe_fileexplorer_navtool_history'],
        { title: $this.Translate('Recent locations') }
      ),
      navtool_up: CreateNode(
        'button',
        ['fe_fileexplorer_navtool_up', 'fe_fileexplorer_disabled'],
        { title: $this.Translate('Up (Alt + Up Arrow)'), tabIndex: -1 }
      ),
      ...
      pathsegmentsscrollwrap: CreateNode('div', [
        'fe_fileexplorer_path_segments_scroll_wrap',
      ]),
      pathsegmentswrap: CreateNode('div', [
        'fe_fileexplorer_path_segments_wrap',
      ]),

      bodywrapouter: CreateNode('div', ['fe_fileexplorer_body_wrap_outer']),
      bodywrap: CreateNode('div', ['fe_fileexplorer_body_wrap']),

      bodytoolsscrollwrap: CreateNode('div', [
      ...
      itemsscrollwrap: CreateNode(
        'div',
        ['fe_fileexplorer_items_scroll_wrap'],
        { tabIndex: 0 }
      ),
      itemsscrollwrapinner: CreateNode('div', [
        'fe_fileexplorer_items_scroll_wrap_inner',
      ]),
      itemsmessagewrap: CreateNode('div', [
        'fe_fileexplorer_items_message_wrap',
      ]),
      itemswrap: CreateNode('div', [
        'fe_fileexplorer_items_wrap',
        'fe_fileexplorer_hidden',
      ]),

      itemsclipboardoverlaypastewrap: CreateNode('div', [
        'fe_fileexplorer_items_clipboard_overlay_paste_wrap',
        'fe_fileexplorer_hidden',
      ]),
      itemsclipboardoverlaypasteinnerwrap: CreateNode('div', [
        'fe_fileexplorer_items_clipboard_overlay_paste_inner_wrap',
      ]),
      itemsclipboardoverlaypastetextwrap: CreateNode('div', [
        'fe_fileexplorer_items_clipboard_overlay_paste_text_wrap',
      ]),
      itemsclipboardoverlaypastetext: CreateNode('div', [
        'fe_fileexplorer_items_clipboard_overlay_paste_text',
      ]),
      itemsclipboardoverlaypastetextline: CreateNode(
        'div',
        ['fe_fileexplorer_items_clipboard_overlay_paste_text_big'],
        { innerHTML: $this.Translate('Paste here') }
      ),
      itemsclipboardoverlaypastetexthint: CreateNode('div', [
        'fe_fileexplorer_items_clipboard_overlay_paste_text_small',
      ]),
      itemsclipboardoverlay: CreateNode(
        'textarea',
      ...
      statusbarmeasuresize: CreateNode('div', [
        'fe_fileexplorer_statusbar_measure_em_size',
      ]),
      statusbartextwrap: CreateNode('div', [
        'fe_fileexplorer_statusbar_text_wrap',
      ]),
      statusbartextsegments: [],
      statusbartextsegmentmap: {},
      statusbaractionwrap: CreateNode('div', ['fe_fileexplorer_action_wrap']),
      statusbaractionprogresswrap: CreateNode('div', [
        'fe_fileexplorer_action_progress_wrap',
        'fe_fileexplorer_hidden',
      ]),
      statusbaractionprogressmessagewrap: CreateNode('div', [
        'fe_fileexplorer_action_progress_msg_wrap',
      ]),
      statusbaractionprogressmessagewrap2: CreateNode('div', [
        'fe_fileexplorer_action_progress_msg_wrap',
        'fe_fileexplorer_action_progress_msg_wrap_last',
      ]),
      statusbaractionprogresscancelwrap: CreateNode(
        'div',
        ['fe_fileexplorer_action_progress_cancel_wrap'],
        { title: $this.Translate('Cancel all'), tabIndex: 0 }
      ),
    };
    ...
    $this.SetNamedStatusBarText = function (name, text, timeout) {
      ...
      if (!(name in elems.statusbartextsegmentmap)) {
        elems.statusbartextsegmentmap[name] = {
          pos: elems.statusbartextsegments.length,
          timeout: null,
        };
      ...
      if (text === '') {
        ...
        if (timeout) {
          ...
          // Recalculate widths.
          var widthmap = [],
            totalwidth = 1.5 * elems.statusbarmeasuresize.offsetWidth;
          for (var x = 0; x < elems.statusbartextsegments.length; x++) {
            ...
            if (elem2.classList.contains('fe_fileexplorer_hidden'))
              ...
              var currstyle =
                elem2.currentStyle || window.getComputedStyle(elem2);
              var elemwidth =
                elem2.offsetWidth +
                parseFloat(currstyle.marginLeft) +
                parseFloat(currstyle.marginRight);
          ...
          for (
            var x = elems.statusbartextsegments.length;
            totalwidth >= elems.statusbartextwrap.offsetWidth && x;
            x--
          ) {
            if (
              widthmap[x - 1] &&
              elem !== elems.statusbartextsegments[x - 1]
            ) {
              elems.statusbartextsegments[x - 1].classList.add(
                'fe_fileexplorer_hidden'
              );
      ...
      elems.statusbartextsegments.forEach(function (elem2) {
        if (!timeout && elem2.innerHTML !== '')
          elem2.classList.remove('fe_fileexplorer_hidden');
      ...
      if (elem)
        elem.classList.add('fe_fileexplorer_statusbar_text_segment_wrap_last');
    };
    ...
    elems.itemsmessagewrap.innerHTML = EscapeHTML(
      $this.Translate('Loading...')
    );

    // Determine what text should show to the user when displaying the paste box.
    if (matchMedia('(pointer: coarse)').matches)
      elems.itemsclipboardoverlaypastetexthint.innerHTML = EscapeHTML(
        $this.Translate('Long-press + paste')
      );
    ...
    elems.itemsclipboardoverlaypastetext.appendChild(
      elems.itemsclipboardoverlaypastetextline
    );
    elems.itemsclipboardoverlaypastetext.appendChild(
      elems.itemsclipboardoverlaypastetexthint
    );
    elems.itemsclipboardoverlaypastetextwrap.appendChild(
      elems.itemsclipboardoverlaypastetext
    );
    elems.itemsclipboardoverlaypasteinnerwrap.appendChild(
      elems.itemsclipboardoverlaypastetextwrap
    );
    elems.itemsclipboardoverlaypasteinnerwrap.appendChild(
      elems.itemsclipboardoverlay
    );
    elems.itemsclipboardoverlaypastewrap.appendChild(
      elems.itemsclipboardoverlaypasteinnerwrap
    );
    ...
    elems.statusbaractionprogresswrap.appendChild(
      elems.statusbaractionprogressmessagewrap
    );
    elems.statusbaractionprogresswrap.appendChild(
      elems.statusbaractionprogressmessagewrap2
    );
    elems.statusbaractionprogresswrap.appendChild(
      elems.statusbaractionprogresscancelwrap
    );
    ...
    $this.SetNamedStatusBarText(
      'folder',
      EscapeHTML($this.Translate('Loading...'))
    );

    // Adjusts the status bar based on how large/small the mainwrap element is.
    var CalculateUpdateMultilineStatus = function () {
      if (
        elems.mainwrap.offsetWidth / elems.statusbarmeasuresize.offsetWidth <
        58
      )
        elems.statusbar.classList.add(
          'fe_fileexplorer_statusbar_wrap_multiline'
        );
      else
        elems.statusbar.classList.remove(
          'fe_fileexplorer_statusbar_wrap_multiline'
        );
    };
    ...
    var statusbarresizewatch = new DebounceAttributes({
      watchers: [{ elem: elems.mainwrap, attr: 'offsetWidth', val: -1 }],
      interval: 50,
      stopsame: 5,
    ...
    if ($this.settings.onfocus)
      $this.addEventListener('focus', $this.settings.onfocus);
    if ($this.settings.onblur)
      $this.addEventListener('blur', $this.settings.onblur);
    if ($this.settings.onrefresh)
      $this.addEventListener('refresh_folder', $this.settings.onrefresh);
    if ($this.settings.onselchanged)
      $this.addEventListener('selections_changed', $this.settings.onselchanged);
    if ($this.settings.onrename)
      $this.addEventListener('rename', $this.settings.onrename);
    if ($this.settings.onopenfile)
      $this.addEventListener('open_file', $this.settings.onopenfile);
    if ($this.settings.oninitupload)
      $this.addEventListener('init_upload', $this.settings.oninitupload);
    if ($this.settings.onuploaderror)
      $this.addEventListener('upload_error', $this.settings.onuploaderror);
    if ($this.settings.onfinishedupload)
      $this.addEventListener('upload_done', $this.settings.onfinishedupload);
    if ($this.settings.ondownloadurl)
      $this.addEventListener('get_download_url', $this.settings.ondownloadurl);
    if ($this.settings.oncopy)
      $this.addEventListener('copy', $this.settings.oncopy);
    if ($this.settings.onmove)
      $this.addEventListener('move', $this.settings.onmove);
    if ($this.settings.ondelete)
      $this.addEventListener('delete', $this.settings.ondelete);
    ...
    var CaptureMouseEnterHandler = function (e) {
      ...
      if (internalcapturerefs === 1) {
        ...
        window.history.pushState(
          { _fileexplorer: 'back', _fileexplorerprevscroll: prevscrollrestore },
          document.title
        );
        window.history.scrollRestoration = 'manual';
    ...
    if ($this.settings.alwaysfocused)
      elems.innerwrap.classList.add('fe_fileexplorer_inner_wrap_focused');
    ...
    var InnerWrapBlurHandler = function (e) {
      ...
      if (node !== elems.mainwrap) {
        if (!$this.settings.alwaysfocused)
          elems.innerwrap.classList.remove(
    ...
    var FocusKeyHandler = function (e) {
      ...
      var node = e.target;
      while (node && node !== elems.itemsscrollwrap && node !== elems.mainwrap)
        node = node.parentNode;

      if (
        node === elems.itemsscrollwrap ||
        e.target === elems.itemsclipboardoverlay
      )
        elems.itemswrap.classList.add('fe_fileexplorer_items_focus');
    ...
    if (matchMedia('(pointer: coarse)').matches)
      elems.innerwrap.classList.add('fe_fileexplorer_show_item_checkboxes');
      ...
      if (e.target.tagName === 'BUTTON') {
        if (e.keyCode == 38) {
          ...
          for (var x = 0; x < elems.bodytools.length; x++) {
            ...
            if (
              !elems.bodytools[x].classList.contains('fe_fileexplorer_disabled')
            )
              lastelem = elems.bodytools[x];
          }
        } else if (e.keyCode == 40) {
          // Down arrow.
          for (var x = elems.bodytools.length; x; x--) {
            ...
            if (
              !elems.bodytools[x - 1].classList.contains(
                'fe_fileexplorer_disabled'
              )
            )
              lastelem = elems.bodytools[x - 1];
          }
        }
      }
    };
    ...
    var ToolbarToolFocusHandler = function (e) {
      ...
      if (node.parentNode === elems.bodytoolbar) {
        if (node.offsetTop - 1 < elems.bodytoolsscrollwrap.scrollTop)
          elems.bodytoolsscrollwrap.scrollTop = node.offsetTop - 1;
        else if (
          node.offsetTop + node.offsetHeight + 1 >
          elems.bodytoolsscrollwrap.scrollTop +
            elems.bodytoolsscrollwrap.clientHeight
        )
          elems.bodytoolsscrollwrap.scrollTop =
            node.offsetTop +
            node.offsetHeight +
    ...
    elems.bodytoolsscrollwrap.addEventListener(
      'focus',
      ToolbarToolFocusHandler,
      true
    );
        ...
        for (var x = 0; x < elems.bodytools.length; x++) {
          if (elems.bodytools[x].tabIndex == 0) {
            ...
            if (
              ...
              for (var x2 = x; x2; x2--) {
                if (
                  !elems.bodytools[x2 - 1].classList.contains(
                    'fe_fileexplorer_disabled'
                  )
                ) {
                  elems.bodytools[x2 - 1].tabIndex = 0;
                  if (document.activeElement === elems.bodytools[x])
                    elems.bodytools[x2 - 1].focus();
              ...
              if (!found) {
                for (var x2 = x + 1; x2 < elems.bodytools.length; x2++) {
                  if (
                    !elems.bodytools[x2].classList.contains(
                      'fe_fileexplorer_disabled'
                    )
                  ) {
                    found = true;
              ...
              if (!found && document.activeElement === elems.bodytools[x])
                $this.Focus(true);
              elems.bodytools[x].tabIndex = -1;
            }
    ...
    $this.ClearSelectedItems = function (ignorebusy, skipuiupdate) {
      ...
      if (currfolder && currfolder.IsBusy() && !ignorebusy) {
        currfolder.AddBusyQueueCallback($this.ClearSelectedItems, [
          ignorebusy,
          skipuiupdate,
        ]);
    ...
    $this.ScrollToFocusedItem = function () {
      if (focuseditem !== false) {
        if (focuseditem.offsetTop - 1 < elems.itemsscrollwrap.scrollTop)
          elems.itemsscrollwrap.scrollTop = focuseditem.offsetTop - 1;
        else if (
          focuseditem.offsetTop + focuseditem.offsetHeight + 1 >
          elems.itemsscrollwrap.scrollTop + elems.itemsscrollwrap.clientHeight
        )
          elems.itemsscrollwrap.scrollTop =
            focuseditem.offsetTop +
            focuseditem.offsetHeight +
            1 -
            elems.itemsscrollwrap.clientHeight;
      }
    };
    ...
    $this.OpenSelectedItems = function () {
      ...
      for (var x in selecteditemsmap) {
        if (selecteditemsmap.hasOwnProperty(x))
          selnums.push(selecteditemsmap[x]);
      }

      selnums.sort(function (a, b) {
        return a < b ? -1 : 1;
      });
    ...
    var ThumbnailDisplayHandler = function (opts, success) {
      ...
      if (!success) queuedthumbfailed[opts.src] = true;
        ...
        if (opts.id in queuedthumbs && queuedthumbs[opts.id] in entryidmap) {
          ...
          var node =
            elems.itemswrap.children[pos].firstChild.firstChild.nextSibling;

          node.className =
            'fe_fileexplorer_item_icon fe_fileexplorer_item_icon_img';
    ...
    var ThumbnailQueueUpdate = function () {
      ...
      if (queuedthumbfolder !== currfolder) {
        ...
        queuedthumbendrow = folderitemcache.rows.length
          ? folderitemcache.rows.length - 1
          : 0;
      }
      ...
      for (var x in queuedthumbs) {
        if (
          !fe_thumbnailloader.IsActive(x) ||
          !(queuedthumbs[x] in entryidmap)
        ) {
          fe_thumbnailloader.RemoveFromQueue(x);
      ...
      while (
        queuedthumbstartrow &&
        (queuedthumbstartrow > folderitemcache.rows.length - 1 ||
          miny < folderitemcache.rows[queuedthumbstartrow].top)
      )
        queuedthumbstartrow--;
      while (
        queuedthumbstartrow < folderitemcache.rows.length - 1 &&
        miny >= folderitemcache.rows[queuedthumbstartrow + 1].top
      )
        queuedthumbstartrow++;

      while (
        queuedthumbendrow &&
        (queuedthumbendrow > folderitemcache.rows.length - 1 ||
          maxy < folderitemcache.rows[queuedthumbendrow].top)
      )
        queuedthumbendrow--;
      if (queuedthumbendrow < queuedthumbstartrow)
        queuedthumbendrow = queuedthumbstartrow;
      while (
        queuedthumbendrow < folderitemcache.rows.length - 1 &&
        maxy >= folderitemcache.rows[queuedthumbendrow + 1].top
      )
        queuedthumbendrow++;
      ...
      for (
        var x = queuedthumbstartrow * folderitemcache.cols.length;
        x < y;
        x++
      ) {
        if (
          entries[x].thumb &&
          (typeof entries[x].thumb !== 'string' ||
            !queuedthumbfailed[entries[x].thumb])
        ) {
    ...
    elems.itemsscrollwrap.addEventListener(
      'scroll',
      thumbnailscrollwatch.Start
    );
    ...
    var UpdateCurrFolderItemCache = function (forcerefresh) {
      if (
        !forcerefresh &&
        folderitemcache &&
        folderitemcache.lastinnerwidth ===
          elems.itemsscrollwrapinner.clientWidth &&
        folderitemcache.size === elems.itemswrap.children.length
      )
        return;
      ...
      while (node) {
        ...
        if (childnode.offsetTop === basey)
          folderitemcache.cols.push({
            left: childnode.offsetLeft,
            right: childnode.offsetLeft + childnode.offsetWidth,
          });
    ...
    var UpdateSelectionsChanged = function () {
      ...
      if (numfound)
        sizestr = GetDisplayFilesize(
          totalsize,
          $this.settings.adjustprecision,
          $this.settings.displayunits
        );
      ...
      if (numselecteditems == 0) $this.SetNamedStatusBarText('selected', '');
        ...
        if (numfound)
          $this.SetNamedStatusBarText(
            ...
            EscapeHTML(
              FormatStr(
                $this.Translate('{0} items selected {1}'),
                numselecteditems,
                sizestr
              )
            )
          ...
          $this.SetNamedStatusBarText(
            'selected',
            EscapeHTML(
              FormatStr($this.Translate('{0} items selected'), numselecteditems)
            )
          );
      } else {
        if (numfound)
          $this.SetNamedStatusBarText(
            'selected',
            EscapeHTML(
              FormatStr($this.Translate('1 item selected {0}'), sizestr)
            )
          );
        else
          $this.SetNamedStatusBarText(
            'selected',
            EscapeHTML($this.Translate('1 item selected'))
          );
      }

      // Notify selection change listeners.
      DispatchEvent('selections_changed', [
        currfolder,
        selecteditemsmap,
        numselecteditems,
      ]);
    ...
    var SyncCurrFolderToDOM = function () {
      ...
      for (x = 0; x < nodes.length; x++) {
        elemmap[nodes[x].dataset.feid] = [
          x,
          nodes[x].dataset.fehash,
          nodes[x],
          false,
        ];
      }
      ...
      if (
        currhistory > 0 &&
        currhistory === historystack.length - 1 &&
        historystack[currhistory].lastselected === true
      ) {
        var currkey =
          historystack[currhistory].folderkeys[
            historystack[currhistory].folderkeys.length - 1
          ];

        for (
          x = 0;
          x < historystack[currhistory - 1].folderkeys.length - 1;
          x++
        ) {
          if (currkey === historystack[currhistory - 1].folderkeys[x]) {
            var temppath =
              foldermap[
                historystack[currhistory - 1].folderkeys[x + 1]
              ].GetPath();

            historystack[currhistory].lastselected =
      ...
      for (
        x = 0;
        x < entries.length &&
        elemmap[entries[x].id] &&
        entries[x].hash === elemmap[entries[x].id][1];
        x++
      ) {
        ...
        if (
          !selchanged &&
          historystack[currhistory].lastselected === node.dataset.feid &&
          !(node.dataset.feid in selecteditemsmap)
        ) {
          node.classList.add('fe_fileexplorer_item_selected');
          node.firstChild.firstChild.checked = true;
      ...
      for (; x < entries.length; x++) {
        ...
        if (entries[x].id in elemmap) {
          ...
          var iteminner = CreateNode(
            'div',
            ['fe_fileexplorer_item_wrap_inner'],
            { tabIndex: -1, draggable: true }
          );
          var itemcheckbox = CreateNode(
            'input',
        ...
        if (entries[x].hash !== node.dataset.fehash) {
          ...
          if (entries[x].type === 'folder') {
            ...
            if (pos < 0)
              itemicon.classList.add('fe_fileexplorer_item_icon_file_no_ext');
            else {
              ...
              if (ext === '')
                ...
                if ((cc >= 48 && cc < 58) || (cc >= 64 && cc < 91))
                  itemicon.classList.add(
                    'fe_fileexplorer_item_icon_ext_' +
                      ext.substring(0, 1).toLowerCase()
                  );
              }
            }
          }
          ...
          if ('overlay' in entries[x]) {
            if (typeof entries[x].overlay === 'string')
              entries[x].overlay = [entries[x].overlay];

            for (var x2 = 0; x2 < entries[x].overlay.length; x2++)
              node.firstChild.classList.add(entries[x].overlay[x2]);
          }

          if ('tooltip' in entries[x])
            node.firstChild.title = entries[x].tooltip;
    ...
    var FinalizeSyncCurrFolderToDOM = function (historyselected, selchanged) {
      ...
      for (var x in selecteditemsmap) {
        if (selecteditemsmap.hasOwnProperty(x) && x in entryidmap)
          selecteditemsmap[x] = entryidmap[x];
      }
      ...
      if (entries.length) {
        ...
        if (currfolder.waiting)
          elems.itemsmessagewrap.innerHTML = EscapeHTML(
            $this.Translate('Loading...')
          );
        else
          elems.itemsmessagewrap.innerHTML = EscapeHTML(
            $this.Translate('This folder is empty.')
          );
      ...
      if (currfolder.waiting)
        ...
        $this.SetNamedStatusBarText(
          'folder',
          EscapeHTML(FormatStr($this.Translate('{0} items'), entries.length))
        );
    ...
    var ConvertVertToHorzScroll = function (e) {
      var mult =
        e.deltaMode == 1
          ? scrolllineheight * 2
          : e.deltaMode == 2
            ? document.documentElement.clientHeight - 2 * scrolllineheight
            : 1;
    ...
    elems.pathsegmentsscrollwrap.addEventListener(
      'wheel',
      ConvertVertToHorzScroll
    );
    ...
    $this.IsValidPath = function (path) {
      ...
      for (var x = 0; x < path.length; x++) {
        if (
          !Array.isArray(path[x]) ||
          path[x].length < 2 ||
          typeof path[x][0] !== 'string' ||
          typeof path[x][1] !== 'string'
        )
          return false;
      }
    ...
    $this.RefreshFolders = function (forcecurrfolder) {
      ...
      for (var x = historyentry.folderkeys.length; x; x--) {
        if (foldermap[historyentry.folderkeys[x - 1]].lastrefresh < ts2) {
          DispatchEvent('refresh_folder', [
            foldermap[historyentry.folderkeys[x - 1]],
            !foldermap[historyentry.folderkeys[x - 1]].lastrefresh,
          ]);
    ...
    var NavigationChanged = function () {
      ...
      if (currhistory <= 0) {
        ...
        var prevpath =
          foldermap[
            historystack[currhistory - 1].folderkeys[
              historystack[currhistory - 1].folderkeys.length - 1
            ]
          ].GetPath();

        elems.navtool_back.title = FormatStr(
          $this.Translate('Back to "{0}" (Alt + Left Arrow)'),
      ...
      if (currhistory >= historystack.length - 1) {
        ...
        elems.navtool_forward.title = $this.Translate(
        ...
        elems.navtool_forward.title = FormatStr(
          $this.Translate('Forward to "{0}" (Alt + Right Arrow)'),
      ...
      if (currpath.length <= 1) {
        if (document.activeElement === elems.navtool_up)
          elems.navtool_history.focus();
        ...
        elems.navtool_up.title = FormatStr(
          $this.Translate('Up to "{0}" (Alt + Up Arrow)'),
          currpath[currpath.length - 2][1]
        );
      }
    ...
    var NavToolsKeyHandler = function (e) {
      ...
      if (e.keyCode == 37) {
        ...
        if (
          ...
          document.activeElement.parentNode === elems.navtools
        ) {
          var node = document.activeElement.previousSibling;
          while (node && node.classList.contains('fe_fileexplorer_disabled'))
            node = node.previousSibling;
    ...
    $this.SetPath = function (newpath) {
      ...
      if (currfolder) {
        currfolder.removeEventListener(
          'set_attributes',
          SetFolderAttributesHandler
        );
        currfolder.removeEventListener('set_entries', SetFolderEntriesHandler);
        currfolder.removeEventListener(
          'remove_entry',
          RemoveFolderEntryHandler
        );
      }

      // Clear existing DOM path segments.
      while (elems.pathsegmentswrap.firstChild)
        elems.pathsegmentswrap.removeChild(elems.pathsegmentswrap.lastChild);
      ...
      for (var x = 0; x < newpath.length; x++) {
        ...
        if (!foldermap[pathkey])
          foldermap[pathkey] = new Folder(newpath.slice(0, x + 1));
        ...
        var segmentwrap = CreateNode(
          'div',
          ['fe_fileexplorer_path_segment_wrap'],
          { tabIndex: x < newpath.length - 1 ? -1 : 0 }
        );
        var segmentpathname = CreateNode(
          'button',
          ['fe_fileexplorer_path_name'],
          { tabIndex: -1 }
        );
        var segmentpathopts = CreateNode(
          'button',
      ...
      elems.pathsegmentsscrollwrap.scrollLeft =
        elems.pathsegmentsscrollwrap.scrollWidth;
      ...
      if (prevfolder !== currfolder) {
        while (elems.itemswrap.firstChild) {
          if (document.activeElement === elems.itemswrap.lastChild.firstChild)
      ...
      if (
        ...
        for (var x = currhistory; x < historystack.length; x++) {
          for (var x2 = 0; x2 < historystack[x].folderkeys.length; x2++) {
            DecrementMappedFolderRefCount(
              foldermap[historystack[x].folderkeys[x2]]
            );
          }
        }
        ...
        for (var x = 0; x < historystack[currhistory].folderkeys.length; x++) {
          DecrementMappedFolderRefCount(
            foldermap[historystack[currhistory].folderkeys[x]]
          );
        }
      }
    ...
    var selectanchorpos = null,
      prevselectrect = null,
      selectbox = null,
      selectchangeinvert = false,
      autoscrolltimer = null,
      lastmouseevent = null;
    var lastselecttouch = null,
      lastmousedownevent = null,
      selectmodemulti = false;
    ...
    var UpdateCalculatedSelectedItemsRect = function (rect, hintrect) {
      ...
      rect.endrow = hintrect
        ? hintrect.endrow
        : folderitemcache.rows.length
          ? folderitemcache.rows.length - 1
          : 0;
      rect.endcol = hintrect
        ? hintrect.endcol
        : folderitemcache.rows.length
          ? folderitemcache.cols.length - 1
          : 0;

      // Adjust startrow and endrow until they fall within two tops.
      while (
        rect.startrow &&
        rect.top < folderitemcache.rows[rect.startrow].top
      )
        rect.startrow--;
      while (
        rect.startrow < folderitemcache.rows.length - 1 &&
        rect.top >= folderitemcache.rows[rect.startrow + 1].top
      )
        rect.startrow++;

      while (rect.endrow && rect.bottom < folderitemcache.rows[rect.endrow].top)
        rect.endrow--;
      if (rect.endrow < rect.startrow) rect.endrow = rect.startrow;
      while (
        rect.endrow < folderitemcache.rows.length - 1 &&
        rect.bottom >= folderitemcache.rows[rect.endrow + 1].top
      )
        rect.endrow++;
      ...
      while (
        rect.startcol &&
        rect.left < folderitemcache.cols[rect.startcol].left
      )
        rect.startcol--;
      while (
        rect.startcol < folderitemcache.cols.length - 1 &&
        rect.left >= folderitemcache.cols[rect.startcol + 1].left
      )
        rect.startcol++;

      while (rect.endcol && rect.right < folderitemcache.cols[rect.endcol].left)
        rect.endcol--;
      if (rect.endcol < rect.startcol) rect.endcol = rect.startcol;
      while (
        rect.endcol < folderitemcache.cols.length - 1 &&
        rect.right >= folderitemcache.cols[rect.endcol + 1].left
      )
        rect.endcol++;
    };
    ...
    var DoesRectOverlapItemRect = function (rect, x, y) {
      return (
        folderitemcache.size &&
        rect.left < folderitemcache.cols[x].right &&
        rect.right >= folderitemcache.cols[x].left &&
        rect.bottom >= folderitemcache.rows[y].top &&
        rect.top < folderitemcache.rows[y].bottoms[x]
      );
    };

    // Updates information and selected items for SelectBoxDragHandler().
    var UpdateSelectBoxDragInfoItems = function (
      currselectrect,
      newpos,
      newwidth,
      newheight,
      ...
      if (!scrolling)
        currselectrect.left =
          newpos.x < selectanchorpos.x ? newpos.x : selectanchorpos.x;
      currselectrect.top =
        newpos.y < selectanchorpos.y ? newpos.y : selectanchorpos.y;
      ...
      if (!scrolling) {
        var minrow =
          currselectrect.startrow < prevselectrect.startrow
            ? currselectrect.startrow
            : prevselectrect.startrow;
        var maxrow =
          currselectrect.endrow > prevselectrect.endrow
            ? currselectrect.endrow
            : prevselectrect.endrow;

        if (currselectrect.left < prevselectrect.left) {
          for (var y = minrow; y <= maxrow; y++) {
            for (
              var x = currselectrect.startcol;
              x <= prevselectrect.startcol;
              x++
            ) {
              if (
                !DoesRectOverlapItemRect(prevselectrect, x, y) &&
                DoesRectOverlapItemRect(currselectrect, x, y)
              ) {
          ...
          for (var y = minrow; y <= maxrow; y++) {
            for (
              var x = prevselectrect.startcol;
              x <= currselectrect.startcol;
              x++
            ) {
              if (
                DoesRectOverlapItemRect(prevselectrect, x, y) &&
        ...
        if (currselectrect.right > prevselectrect.right) {
          for (var y = minrow; y <= maxrow; y++) {
            for (
              var x = prevselectrect.endcol;
              x <= currselectrect.endcol;
              x++
            ) {
              if (
                !DoesRectOverlapItemRect(prevselectrect, x, y) &&
          ...
          for (var y = minrow; y <= maxrow; y++) {
            for (
              var x = currselectrect.endcol;
              x <= prevselectrect.endcol;
              x++
            ) {
              if (
                DoesRectOverlapItemRect(prevselectrect, x, y) &&
      ...
      var mincol =
        currselectrect.startcol < prevselectrect.startcol
          ? currselectrect.startcol
          : prevselectrect.startcol;
      var maxcol =
        currselectrect.endcol > prevselectrect.endcol
          ? currselectrect.endcol
          : prevselectrect.endcol;

      if (currselectrect.top < prevselectrect.top) {
        for (
          var y = currselectrect.startrow;
          y <= prevselectrect.startrow;
          y++
        ) {
        ...
        for (
          var y = prevselectrect.startrow;
          y <= currselectrect.startrow;
          y++
        ) {
          for (var x = mincol; x <= maxcol; x++) {
            if (
      ...
      if (selectbox) {
        ...
        if (e.clientY < rect2.top) {
          ...
          var TempScrollDown = function (e) {
            ...
            if (
              elems.itemsscrollwrap.scrollTop >=
              elems.itemsscrollwrap.scrollHeight -
                elems.itemsscrollwrap.clientHeight
            ) {
              clearInterval(autoscrolltimer);
    ...
    var ClipboardOverlayExitContextMenuHandler = function (e) {
      ...
      elems.itemsclipboardoverlay.removeEventListener(
        'keyup',
        ClipboardOverlayKeyHandler
      );
      window.removeEventListener(
        'mousedown',
        ClipboardOverlayExitContextMenuHandler,
      ...
      if (e && e.target === elems.itemsclipboardoverlay)
        $this.Focus(true, true);

      elems.itemsclipboardoverlaypastewrap.classList.add(
        'fe_fileexplorer_hidden'
      );
      elems.itemsclipboardoverlaypastewrap.classList.remove(
        'fe_fileexplorer_items_show_clipboard_overlay_paste'
      );
    ...
    var ClipboardOverlayContextMenuHandler = function (e) {
      ...
      clipboardmenutimout = setTimeout(function () {
        ...
        elems.itemsclipboardoverlay.addEventListener(
          'input',
          ClipboardOverlayExitContextMenuHandler,
          true
        );
      ...
      window.removeEventListener(
        'contextmenu',
        ClipboardOverlayContextMenuHandler,
        true
      );
    ...
    var StartClipboardOverlayMenu = function () {
      if (
        elems.itemsclipboardoverlaypastewrap.classList.contains(
          'fe_fileexplorer_items_clipboard_contextmenu'
        )
      ) {
        ...
        elems.itemsclipboardoverlay.setSelectionRange(
          numselecteditems ? 1 : 2,
          2
        );
      }
    };
    ...
    var ClipboardPasteBoxMouseHandler = function (e) {
      if (e.target !== elems.itemsclipboardoverlay)
        ClipboardOverlayExitContextMenuHandler(e);
    };
    ...
    var SelectBoxEndHandler = function (e) {
      ...
      elems.itemsscrollwrap.removeEventListener(
        'wheel',
        SelectBoxScrollWheelHandler
      );
    ...
    var StartRenameHandler = function () {
      ...
      if (
        ...
        !currfolder.IsBusy() &&
        numselecteditems === 1 &&
        startrenameid in selecteditemsmap
      )
    ...
    var ScrollNoMoveHandler = function (e) {
      ...
      if (scrollnomoveinfo) {
        if (scrollnomoveinfo.origevent.button == 2) {
          ...
          scrollnomoveinfo.origevent.ctrlKey ||
          scrollnomoveinfo.origevent.target.tagName === 'INPUT' ||
          selectmodemulti
        ) {
    ...
    var MoveCopyDragStartHandler = function (e) {
      ...
      if (scrollnomoveinfo) {
        if (
          scrollnomoveinfo.origevent.ctrlKey ||
          scrollnomoveinfo.origevent.target.tagName === 'INPUT' ||
          scrollnomoveinfo.origevent.button == 2 ||
          selectmodemulti
        ) {
          $this.SetSelectedItems(
            [scrollnomoveinfo.origelem.dataset.feid],
            true
          );
        }
      ...
      e.dataTransfer.setData(
        'application/file-explorer-icon;' +
          JSON.stringify({
            group: main_group,
            numitems: numselecteditems,
            type: icontype,
          }),
        ''
      );
      e.dataTransfer.setData(
        'application/file-explorer-folder',
        JSON.stringify({
          source: main_id,
          path: currfolder.GetPath(),
          ids: ids,
        })
      );
      ...
      if ($this.hasEventListener('get_download_url')) {
        ...
        if (result && result.name && result.url)
          e.dataTransfer.setData(
            'DownloadURL',
            'application/octet-stream:' + result.name + ':' + result.url
          );
      }
    };
    ...
    var GetMoveCopyDragNodeInfo = function (e) {
      ...
      while (
        node &&
        node.parentNode &&
        node.parentNode !== elems.pathsegmentswrap &&
        node.parentNode.parentNode !== elems.itemswrap &&
        node !== elems.innerwrap
      )
        node = node.parentNode;
      ...
      if (node.parentNode.parentNode === elems.itemswrap) {
        if (node.parentNode.classList.contains('fe_fileexplorer_item_folder'))
          return { node: node.parentNode, type: 'item' };
    ...
    var MoveCopyDragUpdateStatusText = function (e, final) {
      ...
      var str =
        e.ctrlKey || movecopydata.dropeffect === 'copy'
          ? final
            ? 'Copying'
            : 'Copy'
          : final
      ...
      if (!movecopydata.dropallowed)
        $this.SetNamedStatusBarText(
          'selected',
          EscapeHTML(
            FormatStr($this.Translate(str + '...'), movecopydata.numitems)
          )
        );
        ...
        $this.SetNamedStatusBarText(
          ...
          EscapeHTML(
            FormatStr(
              $this.Translate(str + ' to "{1}"...'),
              movecopydata.numitems,
        ...
        $this.SetNamedStatusBarText(
          ...
          EscapeHTML(
            FormatStr(
              $this.Translate(str + ' to "{1}"...'),
              movecopydata.numitems,
              path[movecopydata.lastnode._fepos][1]
            )
          )
        );
      } else {
    ...
    var MoveCopyDragEnterHandler = function (e) {
      ...
      if (!movecopyenters) {
        ...
        if (movecopydata !== false) {
          movecopydata.issource = elems.innerwrap.classList.contains(
            'fe_fileexplorer_drag_source'
          );
          ...
          for (var x = 0; x < e.dataTransfer.types.length; x++) {
            if (e.dataTransfer.types[x] === 'Files') {
              ...
              movecopydata = {
                ...
                lasttype: 'error',
                dropallowed: false,
                dropeffect: $this.hasEventListener('init_upload')
                  ? 'copy'
                  : 'none',
              };
      ...
      if (movecopydata !== false) {
        ...
        if (movecopydata.dropeffect !== 'none') {
          ...
          if (
            nodeinfo.node !== movecopydata.lastnode ||
            nodeinfo.type !== movecopydata.lasttype
          ) {
            if (movecopydata.lastnode)
              movecopydata.lastnode.classList.remove(
                'fe_fileexplorer_drag_hover'
              );
            ...
            if (!movecopydata.lastnode)
              movecopydata.dropallowed =
                !movecopydata.issource &&
                movecopydata.lasttype === 'currfolder';
            else {
              movecopydata.lastnode.classList.add('fe_fileexplorer_drag_hover');

              movecopydata.dropallowed =
                !movecopydata.issource ||
                movecopydata.lasttype != 'item' ||
                !(movecopydata.lastnode.dataset.feid in selecteditemsmap);
            }
    ...
    var MoveCopyDragOverHandler = function (e) {
      ...
      if (movecopydata !== false) {
        ...
        if (movecopydata.dropeffect !== 'auto')
          e.dataTransfer.dropEffect = movecopydata.dropeffect;
        else if (e.ctrlKey)
          e.dataTransfer.dropEffect = $this.hasEventListener('copy')
            ? 'copy'
            : 'none';
        else
          e.dataTransfer.dropEffect = $this.hasEventListener('move')
            ? 'move'
            : 'none';

        if (origeffect !== e.dataTransfer.dropEffect)
          MoveCopyDragUpdateStatusText(e);
      }
    };
    ...
    var MoveCopyDragLeaveHandler = function (e) {
      ...
      if (movecopyenters < 1) {
        ...
        if (movecopydata.lastnode)
          movecopydata.lastnode.classList.remove('fe_fileexplorer_drag_hover');
    ...
    var PerformFinalMoveCopy = function (
      issource,
      lasttype,
      lastnode,
      optype,
      ...
      else if (lasttype === 'path')
        targetfolder = $this.GetMappedFolderFromPath(
          currfolder.GetPath().slice(0, lastnode._fepos + 1)
        );
      ...
      var movecopycallback = function (success, selectentries) {
        ...
        if (success === true) {
          ...
          if (
            ...
            var entries2 = [];
            for (var x = 0; x < entries.length; x++) {
              if (!(entries[x].id in selecteditemsmap2))
                entries2.push(entries[x]);
            }

            // Adjust the focused item.
            if (
              focuseditem !== false &&
              focuseditem.dataset.feid in selecteditemsmap2
            ) {
              var node = focuseditem.nextSibling;
              while (node && node.dataset.feid in selecteditemsmap2)
                node = node.nextSibling;
          ...
          $this.SetNamedStatusBarText(
            ...
            EscapeHTML(
              FormatStr(
                $this.Translate(
                  (optype === 'copy' ? 'Copying' : 'Moving') +
                    ' items failed.' +
                    (typeof success === 'string' ? '  {0}' : '')
                ),
                success
              )
            ),
        ...
        // Handle entries.
        if (
          $this.IsMappedFolder(targetfolder) &&
          selectentries &&
          ...
          for (var x = 0; x < selectentries.length; x++) {
            if (selectentries[x].id in entryidmap)
              entries[entryidmap[selectentries[x].id]] = selectentries[x];
    ...
    $this.UpdateProgressText = function () {
      ...
      for (var x = 0; x < progresstrackers.length; x++) {
        ...
        if (progresstrackers[x].showbyterate !== false) {
          ...
          if (tracker.started > progresstrackers[x].started)
            tracker.started = progresstrackers[x].started;
        }
        ...
        if (progresstrackers[x].queuesizeunknown !== false)
          tracker.queuesizeunknown = true;

        tracker.itemsdone += progresstrackers[x].itemsdone;
        tracker.faileditems += progresstrackers[x].faileditems;
      }

      var totalbytesstr = GetDisplayFilesize(
        tracker.totalbytes,
        $this.settings.adjustprecision,
        $this.settings.displayunits
      );
      ...
      if (tracker.showbyterate && currts > tracker.started + 3000) {
        ...
        elems.statusbaractionprogressmessagewrap.innerHTML = EscapeHTML(
          FormatStr('{0}\u00A0\u00A0\u00A0{1}', byteratestr, totalbytesstr)
        );
      } else {
        elems.statusbaractionprogressmessagewrap.innerHTML = EscapeHTML(
          FormatStr('{0}', totalbytesstr)
        );
      }
      ...
      if (tracker.uploading !== false) {
        stropts.push(
          $this.Translate(
            '{0} <span class="fe_fileexplorer_action_progress_msg_icon fe_fileexplorer_action_progress_msg_icon_uploads_in_progress"></span>'
          )
        );
      ...
      if (tracker.queueditems) {
        stropts.push(
          $this.Translate(
            (tracker.queuesizeunknown ? '{1}+ ' : '{1} ') +
              '<span class="fe_fileexplorer_action_progress_msg_icon fe_fileexplorer_action_progress_msg_icon_queued"></span>'
          )
        );
        titleopts.push(
          $this.Translate(
            tracker.queuesizeunknown ? '{1}+ queued' : '{1} queued'
          )
        );
      }
      ...
      if (tracker.itemsdone) {
        stropts.push(
          $this.Translate(
            '{2} <span class="fe_fileexplorer_action_progress_msg_icon fe_fileexplorer_action_progress_msg_icon_done"></span>'
          )
        );
      ...
      if (tracker.faileditems) {
        stropts.push(
          $this.Translate(
            '{3} <span class="fe_fileexplorer_action_progress_msg_icon fe_fileexplorer_action_progress_msg_icon_errors"></span>'
          )
        );
        titleopts.push($this.Translate('{3} failed'));
      }

      elems.statusbaractionprogressmessagewrap2.innerHTML = FormatStr(
        stropts.join('\u00A0\u00A0\u00A0'),
        tracker.uploading,
        tracker.queueditems,
        tracker.itemsdone,
      ...
      elems.statusbaractionprogressmessagewrap2.title = EscapeHTML(
        FormatStr(
          titleopts.join(', '),
          tracker.uploading,
          tracker.queueditems,
    ...
    var ProgressCancelAllHandler = function (e) {
      ...
      $this.SetNamedStatusBarText(
        'message',
        EscapeHTML($this.Translate('Stopping...')),
        $this.settings.messagetimeout
      );

      for (var x = 0; x < progresstrackers.length; x++) {
        if (progresstrackers[x].cancelcallback)
          progresstrackers[x].cancelcallback.call($this, e);
      }

      $this.Focus(true, true);

      elems.statusbaractionprogresscancelwrap.classList.add(
        'fe_fileexplorer_hidden'
      );
    };

    elems.statusbaractionprogresscancelwrap.addEventListener(
      'click',
      ProgressCancelAllHandler
    );
    ...
    $this.CreateProgressTracker = function (cancelcallback) {
      ...
      if (cancelcallback)
        elems.statusbaractionprogresscancelwrap.classList.remove(
          'fe_fileexplorer_hidden'
        );
      elems.statusbaractionprogresswrap.classList.remove(
        'fe_fileexplorer_hidden'
      );
    ...
    var StartUploadChunk = function (fileinfo) {
      ...
      var chunksize = fileinfo.chunksize
        ? typeof fileinfo.chunksize === 'function'
          ? fileinfo.chunksize(fileinfo)
          : fileinfo.chunksize
        : fileinfo.sizebytes - fileinfo.currchunkstart;

      if (fileinfo.currchunkstart + chunksize > fileinfo.sizebytes)
        chunksize = fileinfo.sizebytes - fileinfo.currchunkstart;
      ...
      if (
        !fileinfo.currchunkstart &&
        chunksize < fileinfo.sizebytes &&
        chunksize > 4096
      )
        chunksize = 4096;

      if (fileinfo.type === 'file') {
        fileinfo.currblob = BlobSlice.call(
          fileinfo.file,
          fileinfo.currchunkstart,
          fileinfo.currchunkstart + chunksize,
          fileinfo.file.type
        );
      ...
      var uploadprogresshandler = function (e) {
        ...
        if (currts > fileinfo.progress.started + 3000)
          fileinfo.progress.byterate =
            fileinfo.progress.uploadedbytes /
            (currts - fileinfo.progress.started);

        fileinfo.progress.percent = e.lengthComputable
          ? (e.loaded / e.total) * 100
          : fileinfo.currchunksize
            ? e.loaded / fileinfo.currchunksize
            : 0;
      };
      ...
      var uploadcompletehandler = function (e) {
        ...
        if (fileinfo.progress.uploadedbytes < fileinfo.currchunksize)
          uploadprogresstracker.trackerbytes +=
            fileinfo.currchunksize - fileinfo.progress.uploadedbytes;
      };
        ...
        try {
          ...
          if (!response.success) {
            $this.SetNamedStatusBarText(
              ...
              EscapeHTML(
                FormatStr(
                  $this.Translate('Error uploading "{0}" ({1})'),
                  fileinfo.fullPath,
                  response.errorcode
                )
              ),
            ...
            if (response.entry && $this.IsMappedFolder(origcurrfolder))
              origcurrfolder.SetEntry(response.entry);
            ...
            var finalizecallback = function (success, entry) {
              ...
              if (entry && $this.IsMappedFolder(origcurrfolder))
                origcurrfolder.SetEntry(entry);
      ...
      if (
        fileinfo.type === 'file' &&
        (fileinfo.currchunkstart || fileinfo.currchunksize < fileinfo.sizebytes)
      ) {
        // Handle zero byte chunks gracefully (0 - 1 + 1 = 0).
        if (!fileinfo.currchunksize)
          fileinfo.headers['Content-Range'] = 'bytes 1-0/' + fileinfo.sizebytes;
        else
          fileinfo.headers['Content-Range'] =
            'bytes ' +
            fileinfo.currchunkstart +
            '-' +
            (fileinfo.currchunkstart + fileinfo.currchunksize - 1) +
            '/' +
            fileinfo.sizebytes;

        // Enable FileUploadHelper compatibility.  Only really needs to exist.
        fileinfo.headers['Content-Disposition'] =
          'attachment; filename="' +
          encodeURI(fileinfo.name || fileinfo.file.name) +
          '"';
      }
      ...
      if (fileinfo.currpathparam) {
        formdata.append(
          fileinfo.currpathparam,
          JSON.stringify(currfolder.GetPathIDs())
        );
      }
    ...
    var InitializeUpload = function (fileinfo) {
      ...
      var startuploadcallback = function (process) {
        ...
        if (process === true && !destroyinprogress) {
          ...
          fileinfo.sizebytes =
            fileinfo.type === 'file' ? fileinfo.file.size : 0;
          ...
          if (!uploadprogresstracker) {
            uploadprogresstracker = $this.CreateProgressTracker(
              UploadCancelAllHandler
            );
            uploadprogresstracker.showbyterate = true;
          }

          if (!uploadfolderinterval)
            uploadfolderinterval = setInterval(
              UpdateUploadRefreshFolder,
              30000
            );
          ...
          $this.SetNamedStatusBarText(
            ...
            EscapeHTML(
              FormatStr(
                $this.Translate('Starting "{0}" failed.  {1}'),
                fileinfo.fullPath,
                process
              )
            ),
    ...
    var StartNextUpload = function () {
      ...
      if (uploadcancel && uploaddirentries.length && !uploaddirentries[0].busy)
        uploaddirentries = [];
      ...
      if (
        ...
        uploaddirentries.length &&
        !uploaddirentries[0].busy
      ) {
        ...
        uploaddirentries[0].reader.readEntries(
          function (entries) {
      ...
      if (
        uploadactivequeue.length < $this.settings.concurrentuploads &&
        uploadqueue.length
      ) {
        ...
        for (var x = 0; x < uploadqueue.length; x++) {
          if (uploadqueue[x].status === 'waiting') waiting++;
            ...
            if (uploadqueue[x].status === 'invalid_item')
              console.error(
                "Removed '" +
                  uploadqueue[x].fullPath +
                  "' from the upload queue due to an error - it is probably a directory."
              );
            ...
            if (
              uploadqueue[x].type === 'file' ||
              uploadqueue[x].type === 'dir'
            ) {
      ...
      if (
        ...
        $this.RemoveProgressTracker(
          uploadprogresstracker,
          uploadcancel || destroyinprogress
            ? 'Uploading stopped'
            : 'Uploading done'
        );
    ...
    $this.ProcessFilesAndUpload = function (targettype, targetnode, dt) {
      ...
      if (targettype === 'currfolder') targetfolder = currfolder;
        ...
        targetfolder = $this.GetMappedFolderFromPath(
          currfolder.GetPath().slice(0, targetnode._fepos + 1)
        );
      ...
      if (dt.items && dt.items.length) {
        ...
        for (var x = 0; x < dt.items.length; x++) {
          if (
            typeof dt.items[x].getAsEntry === 'function' &&
            (entry = dt.items[x].getAsEntry())
          )
            QueueUploadEntry(targetfolder, entry);
          else if (
            typeof dt.items[x].webkitGetAsEntry === 'function' &&
            (entry = dt.items[x].webkitGetAsEntry())
          )
            QueueUploadEntry(targetfolder, entry);
          else {
    ...
    var MoveCopyDropHandler = function (e) {
      ...
      if (movecopydata !== false) {
        ...
        if (movecopydata.lasttype !== 'error') {
          ...
          try {
            dropdata = JSON.parse(
              e.dataTransfer.getData('application/file-explorer-folder')
            );
          } catch (e) {}
          ...
          if (dropdata && movecopydata.dropallowed) {
            ...
            PerformFinalMoveCopy(
              movecopydata.issource,
              movecopydata.lasttype,
              movecopydata.lastnode,
...
*/
    ...
    var StartSelectionHandler = function (e) {
      ...
      if (e.type === 'touchstart') {
        ...
        if (
          e.target.classList.contains('fe_fileexplorer_item_wrap_inner') ||
          e.target.closest('.fe_fileexplorer_item_wrap_inner')
        ) {
          ...
          if (
            ...
            startrename =
              !currfolder.IsBusy() &&
              !e.shiftKey &&
              $this.hasEventListener('rename') &&
              numselecteditems === 1 &&
              elem.dataset.feid in selecteditemsmap &&
              e.target.classList.contains('fe_fileexplorer_item_text');
          ...
          if (
            ...
            (e.detail > 1 ||
              (lastselecttouch &&
                origselecteditem &&
                lastmousedownevent &&
                e.timeStamp - lastmousedownevent.timeStamp < 500))
          ) {
...
*/
          ...
          if (
            e.detail == 2 ||
            (e.detail < 2 && (!selectmodemulti || numselecteditems === 1))
          ) {
    ...
    elems.itemsscrollwrapinner.addEventListener(
      'mousedown',
    ...
    var CheckboxSelectedFixHandler = function (e) {
      ...
      if (e.target.tagName === 'INPUT') {
        ...
        e.target.checked = e.target.parentNode.parentNode.classList.contains(
    ...
    var RecentLocationsHandler = function (e) {
      ...
      if (e.type === 'mousedown') {
        var blockpopup = elems.navtool_history.classList.contains(
          'fe_fileexplorer_block_popup'
        );

        if (blockpopup)
          elems.navtool_history.classList.remove('fe_fileexplorer_block_popup');
      ...
      var options = {
        ...
        onposition: function (popupelem) {
          ...
          popupelem.style.top =
            elems.toolbar.offsetTop + elems.toolbar.offsetHeight + 1 + 'px';
        },
        ...
        oncancel: function (lastelem, etype) {
          ...
          if (etype === 'mouse' && lastelem === elems.navtool_history)
            elems.navtool_history.classList.add('fe_fileexplorer_block_popup');
      ...
      for (var x = minnum; x <= maxnum; x++) {
        var temppath =
          foldermap[
            historystack[x].folderkeys[historystack[x].folderkeys.length - 1]
          ].GetPath();
        var item = {
          id: x,
          name: EscapeHTML(temppath[temppath.length - 1][1]),
        };

        if (x < currhistory) item.icon = 'fe_fileexplorer_popup_item_icon_back';
        else if (x > currhistory)
          item.icon = 'fe_fileexplorer_popup_item_icon_forward';
    ...
    elems.navtool_history.addEventListener(
      'keydown',
      RecentLocationsKeyHandler
    );
    ...
    var StartPathSegmentFolderSelection = function (node) {
      var posx =
        node.lastChild.offsetLeft -
        elems.pathsegmentsscrollwrap.scrollLeft +
        elems.pathsegmentsscrollwrap.offsetLeft -
        18;
      ...
      var options = {
        ...
        resizewatchers: [{ elem: document.body, attr: 'offsetWidth', val: -1 }],

        onposition: function (popupelem) {
          var posx2 =
            posx + popupelem.offsetWidth < document.body.offsetWidth
              ? posx
              : document.body.offsetWidth - popupelem.offsetWidth - 1;

          popupelem.style.left = posx2 + 'px';
          popupelem.style.top =
            elems.toolbar.offsetTop + elems.toolbar.offsetHeight + 'px';
        },
        ...
        oncancel: function (lastelem, etype) {
          ...
          if (
            etype === 'mouse' &&
            lastelem.classList.contains('fe_fileexplorer_path_opts')
          )
            node.classList.add('fe_fileexplorer_block_popup');
        ...
        onleft: function (lastelem) {
          ...
          if (pos) {
            ...
            StartPathSegmentFolderSelection(
              elems.pathsegmentswrap.children[pos - 1]
            );
          }
        },
      ...
      if (entries) {
        ...
        for (var x = 0; x < entries.length; x++) {
          if (entries[x].type === 'folder') {
            var item = {
              id: options.items.length,
              name: EscapeHTML(entries[x].name),
              icon: 'fe_fileexplorer_popup_item_icon_folder',
              info: entries[x],
            };
    ...
    var PathSegmentMouseFocusHandler = function (e) {
      ...
      var tabnode =
        elems.pathsegmentswrap.children[elems.pathsegmentswrap._fepos];
      ...
      if (node && node === elems.pathsegmentsscrollwrap && tabnode)
        tabnode.focus();
    ...
    var PathSegmentFocusScrollHandler = function (e) {
      ...
      if (node.parentNode === elems.pathsegmentswrap) {
        if (elems.pathsegmentswrap._fepos !== node._fepos) {
          ...
          elems.pathsegmentswrap.children[
            elems.pathsegmentswrap._fepos
          ].tabIndex = -1;

          elems.pathsegmentswrap._fepos = node._fepos;
        }

        if (node.offsetLeft - 1 < elems.pathsegmentsscrollwrap.scrollLeft)
          elems.pathsegmentsscrollwrap.scrollLeft = node.offsetLeft - 1;
        else if (
          node.offsetLeft + node.offsetWidth + 1 >
          elems.pathsegmentsscrollwrap.scrollLeft +
            elems.pathsegmentsscrollwrap.clientWidth
        )
          elems.pathsegmentsscrollwrap.scrollLeft =
            node.offsetLeft +
            node.offsetWidth +
    ...
    var ClipboardCutCopyHandler = function (e) {
      ...
      if (numselecteditems) {
        ...
        e.clipboardData.setData(
          'application/file-explorer-clipboard',
          JSON.stringify({
            type: dropeffect,
            source: main_id,
            group: main_group,
        ...
        $this.SetNamedStatusBarText(
          ...
          EscapeHTML(
            $this.Translate(
              (e.type === 'copy' ? 'Copy' : 'Cut') + ' to clipboard completed'
            )
          ),
          $this.settings.messagetimeout
        );
    ...
    var ClipboardPasteHandler = function (e) {
      ...
      try {
        dropdata = JSON.parse(
          e.clipboardData.getData('application/file-explorer-clipboard')
        );
      } catch (err) {
        try {
          ...
          if (data['application/file-explorer-clipboard'])
            dropdata = data['application/file-explorer-clipboard'];
        } catch (err) {}
      }

      if (
        dropdata &&
        dropdata.group === main_group &&
        Array.isArray(dropdata.path) &&
        Array.isArray(dropdata.ids) &&
        dropdata.ids.length &&
        ((dropdata.type === 'copy' && $this.hasEventListener('copy')) ||
          (dropdata.type === 'move' && $this.hasEventListener('move')))
      ) {
        var lasttype = 'currfolder',
          lastnode = null;
        ...
        if (
          ...
          focuseditem !== false &&
          focuseditem.classList.contains('fe_fileexplorer_item_folder')
        ) {
          lasttype = 'item';
        ...
        if (dropdata && dropdata.group !== main_group)
          $this.SetNamedStatusBarText(
            'message',
            EscapeHTML(
              $this.Translate(
          ...
          $this.SetNamedStatusBarText(
            'message',
            EscapeHTML($this.Translate('Paste operation failed')),
            $this.settings.messagetimeout
          );
      }
      ...
      if (e.keyCode == 37) {
        ...
        if (focuseditem !== false) {
          if (focuseditem.previousSibling) {
            $this.SetFocusItem(
              focuseditem.previousSibling.dataset.feid,
          ...
          $this.SetFocusItem(
            elems.itemswrap.firstChild.dataset.feid,
            !e.ctrlKey && !e.shiftKey
          );
        ...
        if (focuseditem !== false) {
          if (focuseditem.nextSibling) {
            $this.SetFocusItem(
              focuseditem.nextSibling.dataset.feid,
              !e.ctrlKey && !e.shiftKey
            );
        ...
        if (focuseditem !== false) {
          if (
            focuseditem.offsetTop !==
            folderitemcache.rows[folderitemcache.rows.length - 1].top
          ) {
            var num = folderitemcache.cols.length;
            var node = focuseditem;
        ...
        if (folderitemcache.size) {
          $this.SetFocusItem(
            elems.itemswrap.lastChild.dataset.feid,
            !e.ctrlKey && !e.shiftKey
          );
        ...
        elems.itemsclipboardoverlay.classList.add(
          'fe_fileexplorer_items_clipboard_contextmenu'
        );
    ...
    var ItemsKeypressHandler = function (e) {
      ...
      var entryidmap = currfolder.GetEntryIDMap();
      var node =
        focuseditem !== false && focuseditem.nextSibling
          ? focuseditem.nextSibling
          : elems.itemswrap.firstChild;
      var pos = entryidmap[node.dataset.feid];

      while (node) {
        if (
          lasttypingstr.localeCompare(
            entries[pos].name.substring(0, lasttypingstr.length),
            undefined,
            { sensitivity: 'base' }
          ) === 0
        ) {
    ...
    var HistoryBackKeyHandler = function (e) {
      ...
      if (e.keyCode == 13 || e.keyCode == 32) {
        ...
        else if (currhistory < historystack.length - 1)
          elems.navtool_forward.focus();
        else elems.navtool_history.focus();
      }
    };
    ...
    elems.navtool_forward.addEventListener(
      'mouseup',
      ClickHistoryForwardHandler
    );
    ...
    $this.StartOperationIndicator = function () {
      ...
      operationtimeout = setTimeout(function () {
        elems.innerwrap.classList.remove(
          'fe_fileexplorer_operation_in_progress'
        );
    ...
    $this.RenameSelectedItem = function () {
      ...
      if (
        !currfolder ||
        currfolder.waiting ||
        ('canmodify' in currfolder.GetAttributes() &&
          !currfolder.GetAttributes().canmodify)
      )
        return;
      ...
      var options = {
        ...
        onposition: function (textelem) {
          ...
          var finalheight = Math.min(
            elems.itemsscrollwrap.offsetHeight - 1,
            textelem.scrollHeight + 2
          );
          textelem.style.top =
            (node.offsetTop +
              node.firstChild.lastChild.offsetTop -
              2 +
              finalheight <=
            elems.itemsscrollwrap.scrollTop +
              elems.itemsscrollwrap.offsetHeight -
              2
              ? node.offsetTop + node.firstChild.lastChild.offsetTop - 2
              : elems.itemsscrollwrap.scrollTop +
                elems.itemsscrollwrap.offsetHeight -
                2 -
                finalheight) + 'px';
        ...
        ondone: function (val, lastelem, etype) {
          ...
          var renamecallback = function (newentry) {
            ...
            if (typeof newentry === 'object') {
              node.firstChild.lastChild.classList.remove(
              ...
              if (
                lastelem &&
                focuseditem &&
                lastelem === focuseditem.firstChild
              ) {
                lastelem.focus();
              ...
              if ($this.IsMappedFolder(origfolder)) {
                ...
                if (origentry.id !== newentry.id) {
                  if (currfolder === origfolder) {
                    if (
                      focuseditem !== false &&
                      origentry.id === focuseditem.dataset.feid
                    ) {
                    ...
                    if (origentry.id in selecteditemsmap)
                      $this.SetSelectedItems([newentry.id], true);
                  }
              ...
              $this.SetNamedStatusBarText(
                ...
                EscapeHTML(
                  FormatStr(
                    $this.Translate(
                      'Renaming "{0}" to "{1}" failed.' +
                        (typeof newentry === 'string' ? '  {2}' : '')
                    ),
                    origentry.name,
                    val,
                    newentry
          ...
          $this.SetNamedStatusBarText(
            ...
            EscapeHTML(
              FormatStr(
                $this.Translate('Renaming "{0}" to "{1}"...'),
                origentry.name,
                val
              )
            ),
          ...
          DispatchEvent('rename', [
            renamecallback,
            currfolder,
            Object.assign({}, origentry),
            val,
          ]);
        },
      ...
      textareaoverlayinst = new TextareaOverlay(
        elems.itemsscrollwrapinner,
        options
      );
    };
    ...
    $this.DeleteSelectedItems = function (recycle) {
              ...
              for (var x in selecteditemsmap2) {
                if (selecteditemsmap2.hasOwnProperty(x)) {
                  if (focuseditem !== false && focuseditem.dataset.feid === x) {
                    ...
                    else if (focuseditem.previousSibling)
                      $this.SetFocusItem(
                        focuseditem.previousSibling.dataset.feid,
                        true
                      );
      ...
      if (numselecteditems === 1)
        $this.SetNamedStatusBarText(
          'message',
          EscapeHTML($this.Translate('Deleting 1 item...')),
          $this.settings.messagetimeout
        );
      else
        $this.SetNamedStatusBarText(
          ...
          EscapeHTML(
            FormatStr(
              $this.Translate('Deleting {0} items...'),
              numselecteditems
            )
          ),
          $this.settings.messagetimeout
    ...
    $this.Destroy = function () {
      ...
      for (var x in elems.statusbartextsegmentmap) {
        if (
          elems.statusbartextsegmentmap.hasOwnProperty(x) &&
          elems.statusbartextsegmentmap[x].timeout
        ) {
      ...
      elems.mainwrap.removeEventListener(
        'mouseenter',
        CaptureMouseEnterHandler
      );
      elems.mainwrap.removeEventListener(
        'mouseleave',
        CaptureMouseLeaveHandler
      );

      elems.mainwrap.removeEventListener(
        'mousedown',
        MainWrapFocusMouseHandler
      );
      ...
      elems.bodytoolsscrollwrap.removeEventListener(
        'focus',
        ToolbarToolFocusHandler,
        true
      );
      ...
      elems.pathsegmentsscrollwrap.removeEventListener(
        'wheel',
        ConvertVertToHorzScroll
      );
      ...
      elems.statusbaractionprogresscancelwrap.removeEventListener(
        'click',
        ProgressCancelAllHandler
      );
      elems.statusbaractionprogresscancelwrap.removeEventListener(
      ...
      elems.itemsscrollwrapinner.removeEventListener(
        'mousedown',
        StartSelectionHandler
      );
      elems.itemsscrollwrapinner.removeEventListener(
      ...
      elems.navtool_history.removeEventListener(
        'mousedown',
        RecentLocationsHandler
      );
      elems.navtool_history.removeEventListener(
        'keydown',
        RecentLocationsKeyHandler
      ...
      elems.navtool_back.removeEventListener(
        'mouseup',
        ClickHistoryBackHandler
      );
      elems.navtool_back.removeEventListener('keydown', HistoryBackKeyHandler);
      elems.navtool_forward.removeEventListener(
        'mouseup',
        ClickHistoryForwardHandler
      );
      elems.navtool_forward.removeEventListener(
        'keydown',
        HistoryForwardKeyHandler
      );
      ...
      for (var node in elems) {
        if (Array.isArray(elems[node])) {
          for (var x = 0; x < elems[node].length; x++) {
            if (elems[node][x].parentNode)
              elems[node][x].parentNode.removeChild(elems[node][x]);
          }
    ...
    var UpdateLastUsedToolButtonHandler = function (e) {
      ...
      if (!e.target.classList.contains('fe_fileexplorer_disabled')) {
        elems.bodytools.forEach(function (elem) {
          elem.tabIndex =
            elem === e.target &&
            !elem.classList.contains('fe_fileexplorer_disabled')
              ? 0
              : -1;
        });
      }
    ...
    $this.AddToolbarButton = function (classname, title) {
      var node = CreateNode('button', [classname, 'fe_fileexplorer_disabled'], {
        title: title,
        tabIndex: -1,
      });
      ...
      if (numtoolbuttons && !numtoolbuttongroups)
        elems.bodytoolbar.appendChild(
          CreateNode('div', ['fe_fileexplorer_folder_tool_separator'])
        );
    ...
    if (numtoolbuttons)
      elems.bodytoolsscrollwrap.classList.remove('fe_fileexplorer_hidden');
  ...
  var FileExplorerTool_NewFolder = function (fe) {
    if (!(this instanceof FileExplorerTool_NewFolder))
      return new FileExplorerTool_NewFolder(fe);
    ...
    var node = fe.AddToolbarButton(
      'fe_fileexplorer_folder_tool_new_folder',
      fe.Translate('New Folder (Ctrl + Ins)')
    );

    if (fe.settings.onnewfolder)
      fe.addToolEventListener('new_folder', fe.settings.onnewfolder);
    ...
    var CreateFolder = function () {
        ...
        if (typeof newentry === 'object') {
          ...
          fe.SetNamedStatusBarText(
            ...
            EscapeHTML(
              FormatStr(
                fe.Translate(
                  'Creating a new folder failed.' +
                    (typeof newentry === 'string' ? '  {0}' : '')
                ),
                newentry
              )
            ),
            fe.settings.messagetimeout
          );
        }
      };
      ...
      fe.SetNamedStatusBarText(
        'message',
        EscapeHTML(fe.Translate('Creating a new folder...')),
        fe.settings.messagetimeout
      );
    ...
    var UpdateToolHandler = function (currfolder, attrs) {
      ...
      enabled =
        !currfolder.waiting && (!('canmodify' in attrs) || attrs.canmodify);
  ...
  var FileExplorerTool_NewFile = function (fe) {
    if (!(this instanceof FileExplorerTool_NewFile))
      return new FileExplorerTool_NewFile(fe);
    ...
    var node = fe.AddToolbarButton(
      'fe_fileexplorer_folder_tool_new_file',
      fe.Translate('New File (Ins)')
    );

    if (fe.settings.onnewfile)
      fe.addToolEventListener('new_file', fe.settings.onnewfile);
    ...
    var CreateFile = function () {
      ...
      fe.SetNamedStatusBarText(
        'message',
        EscapeHTML(fe.Translate('Creating a new file...')),
        fe.settings.messagetimeout
      );
  ...
  var FileExplorerTool_Upload = function (fe) {
    if (!(this instanceof FileExplorerTool_Upload))
      return new FileExplorerTool_Upload(fe);

    // Do not create the tool if uploading is disabled.
    if (!fe.hasEventListener('init_upload') && !fe.settings.tools.upload)
      return;
    ...
    var CreateInputField = function () {
      ...
      inputfield = CreateNode('input', ['fe_fileexplorer_hidden'], {
        type: 'file',
        multiple: true,
      });
  ...
  var FileExplorerTool_Download = function (fe) {
    if (!(this instanceof FileExplorerTool_Download))
      return new FileExplorerTool_Download(fe);
    ...
    var node = fe.AddToolbarButton(
      ...
      fe.Translate('Download')
    );

    if (fe.settings.oninitdownload)
      fe.addToolEventListener('init_download', fe.settings.oninitdownload);
    if (fe.settings.ondownloadstarted)
      fe.addToolEventListener(
        'download_started',
        fe.settings.ondownloadstarted
      );
    if (fe.settings.ondownloaderror)
      fe.addToolEventListener('download_error', fe.settings.ondownloaderror);
    ...
    var ClickHandler = function (e) {
      ...
      var downloadcallback = function (options) {
        ...
        if (typeof options === 'object') {
          ...
          var formnode = CreateNode('form', ['fe_fileexplorer_hidden'], {
            action: options.url,
            method: method,
            ...
            if (options.params instanceof FormData) {
              ...
              for (var x = 0; x < options.params.length; x++)
                formnode.appendChild(
                  CreateNode('input', [], {
                    type: 'hidden',
                    name: options.params[x].name,
                    value: options.params[x].value,
                  })
                );
            } else {
              for (var x in options.params) {
                if (
                  options.params.hasOwnProperty(x) &&
                  typeof options.params[x] === 'string'
                )
                  formnode.appendChild(
                    CreateNode('input', [], {
          ...
          if (!('iframe' in options) || options.iframe) {
            var iframenode = CreateNode('iframe', [
            ...
            var iframeloadhandler = function (e) {
              if (formnode) {
                ...
                fe.SetNamedStatusBarText(
                  'message',
                  EscapeHTML(
                    FormatStr(fe.Translate('Download failed to start'))
                  ),
                  fe.settings.messagetimeout
                );
          ...
          fe.SetNamedStatusBarText(
            'message',
            EscapeHTML(FormatStr(fe.Translate('Starting download...'))),
            fe.settings.messagetimeout
          );
        } else {
          fe.StopOperationIndicator();
      ...
      fe.SetNamedStatusBarText(
        'message',
        EscapeHTML(fe.Translate('Initializing download...')),
        fe.settings.messagetimeout
      );

      fe.DispatchToolEvent('init_download', [
        downloadcallback,
        fe.GetCurrentFolder(),
        fe.GetSelectedItemIDs(),
        fe.GetSelectedFolderEntries(),
      ]);
    };
  ...
  var FileExplorerTool_Copy = function (fe) {
    if (!(this instanceof FileExplorerTool_Copy))
      return new FileExplorerTool_Copy(fe);
    ...
    var node = fe.AddToolbarButton(
      'fe_fileexplorer_folder_tool_copy',
      fe.Translate(
        'Copy (' +
  ...
  var FileExplorerTool_Paste = function (fe) {
    if (!(this instanceof FileExplorerTool_Paste))
      return new FileExplorerTool_Paste(fe);

    // Do not create the tool if copying and moving are disabled.
    if (
      !fe.hasEventListener('copy') &&
      !fe.hasEventListener('move') &&
      !fe.settings.tools.paste
    )
      return;
  ...
  var FileExplorerTool_Cut = function (fe) {
    if (!(this instanceof FileExplorerTool_Cut))
      return new FileExplorerTool_Cut(fe);
    ...
    var UpdateToolHandler = function (currfolder, attrs) {
      ...
      enabled =
        !currfolder.waiting &&
        (!('canmodify' in attrs) || attrs.canmodify) &&
        fe.GetNumSelectedItems() > 0;
  ...
  var FileExplorerTool_Delete = function (fe) {
    if (!(this instanceof FileExplorerTool_Delete))
      return new FileExplorerTool_Delete(fe);
  ...
  var FileExplorerTool_ItemCheckboxes = function (fe) {
    if (!(this instanceof FileExplorerTool_ItemCheckboxes))
      return new FileExplorerTool_ItemCheckboxes(fe);

    // Do not create the tool if the device's primary input is not coarse and the tool is not specifically enabled.
    if (
      !matchMedia('(pointer: coarse)').matches &&
      !fe.settings.tools.item_checkboxes
    )
      return;

    var node = fe.AddToolbarButton(
      'fe_fileexplorer_folder_tool_item_checkboxes',
      fe.Translate('Item Checkboxes')
    );
    ...
    var ClickHandler = function (e) {
      ...
      fe.GetElements().innerwrap.classList.toggle(
